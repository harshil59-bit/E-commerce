    ...
    <main className="grid min-h-screen place-items-center bg-slate-100 px-4 py-10">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-slate-600">Sign in to shop products, carts, and orders.</p>
        </div>
        <div className="grid gap-4">
          ...
          <label className="grid gap-1 text-sm font-semibold">
            Password
            <input type="password" required value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} className="rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-brand" />
          </label>
        ...
        <p className="mt-5 text-center text-sm text-slate-600">
          New to CartLabs? <Link className="font-bold text-brand" to="/register">Create account</Link>
        </p>
      </form>
    </main>
  );
